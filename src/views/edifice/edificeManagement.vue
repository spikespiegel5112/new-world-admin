      ...
      <el-col :span="20">
        <el-form :rules="rules" ref="queryModel" :model="queryModel" label-position="right" label-width="150px">
          <el-form-item label="游戏名称" prop="name">
            <el-autocomplete v-model="chosenGameName" :fetch-suggestions="focusSortList" placeholder="请输入内容" @select="chooseGame"></el-autocomplete>
          </el-form-item>
        </el-form>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    getGameTypeInfo() {
      ...
      this.$http
        .get(this.$baseUrl + this.game_typeListRequest, {
    ...
    getSortList() {
      ...
      console.log(result);
      this.$http
        .get(this.$baseUrl + this.rankListRequest, {
          headers: {
    ...
    chooseDeviceType(data) {
      console.log(data);
      this.currentDeviceType = this.$store.state.app.deviceTypeDictionary[
        data.index
      ].code;
      this.queryModel = Object.assign(this.queryModel, {
        gameTypeId: this.currentGameTypeId,
    ...
    saveSort() {
      ...
      this.$http
        .post(
          this.$baseUrl +
          this.rankGameByTypeRequest +
          `/${this.currentGameTypeId}`,
          requestBody
        )
        .then(response => {
          console.log(response);
    ...
    focusSortList(queryString, callback) {
      ...
      this.$http
        .get(this.$baseUrl + this.game_infoListRequest, {
          params: this.queryModel
        })
        .then(response => {
          console.log(response);
